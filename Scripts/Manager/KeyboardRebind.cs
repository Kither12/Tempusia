...
public class KeyboardRebind : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        {
            KeyboardManager.LoadBindingOveride(actionName);
            GetBindingInfo();
            UpdateUI();
            UpdateDependence();
        }
    ...
    private void UpdateUI()
    {
        ...
        {
            HandleTextUI(KeyboardManager.getBindingName(actionName, bindingIndex));
        }
        else
        {
    ...
    private void UpdateDependence()
    {
        ...
        {
            KeyboardManager.input.FindAction(dependInput.action.name).ApplyBindingOverride(0, KeyboardManager.input.FindAction(actionName).bindings[bindingIndex]);
        }
    }
    private void DoRebind()
    {
        KeyboardManager.DoRebind(actionName, bindingIndex, otherKeys);
    }
    ...
    {
        KeyboardManager.ResetBinding(actionName, bindingIndex);
        UpdateUI();
        UpdateDependence();
    }
}
